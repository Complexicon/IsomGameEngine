...
enum class UserInputType { Mouse, Keyboard };
...
class KeyDownEvent : public KeyboardInputEvent {
  public:
	KeyDownEvent(Key key) : KeyboardInputEvent(key, true) {}
};

class KeyUpEvent : public KeyboardInputEvent {
  public:
	KeyUpEvent(Key key) : KeyboardInputEvent(key, false) {}
};
...
class MouseClickEvent : public MouseInputEvent {
  public:
	...
	MouseClickEvent(MouseButton b, int x, int y, bool pressed)
		: button(b), pressed(pressed), pressedX(x), pressedY(y), MouseInputEvent(MouseEventType::Click) {}
};